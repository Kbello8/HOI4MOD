...
NDiplomacy = {
	...
	RESOURCE_SENT_AUTONOMY_DAILY_FACTOR = 0.03,	-- If puppet provides resources to its master they increasy their autonomy by the resources factored by this
	...
	LL_TO_OVERLORD_AUTONOMY_DAILY_FACTOR = 0.2,	-- If puppet lend leases equipment to overlord of at least same tech level as they have, they gain autonomy
...
NPolitics = {
	...
	ARMY_LEADER_COST = 4,					-- cost for recruiting new leaders, 'this value' * number_of_existing_leaders_of_type
	NAVY_LEADER_COST = 4,					-- cost for recruiting new leaders, 'this value' * number_of_existing_leaders_of_type
...
NMilitary = {
	...
	TRAINING_ATTRITION = 0.005,		  			   -- amount of extra attrition from being in training
	...
	FRONT_MIN_PATH_TO_REDEPLOY = 20,					-- If a units path is at least this long to reach its front location, it will strategically redeploy.
	...
	HOURS_REQ_REJOIN_BORDER_WAR_FOR_INJURED_UNITS = 720, -- minimum hours required for units to rejoin border wars
...
NAir = {
	...
	AGGRESSION_THRESHOLD = { 0.25, 0.5, 0.75 },			-- Threshold levels for mission aggressivity for air
...
NNavy = {
	...
	COMBAT_MIN_WIN_CHANCE_TO_KEEP_FIGHTING = 0.8,		 			-- compare our forces with enemy forces. 1.0 is equal win chances. Less then 1.0 is we have low chances. If we have at least X chances, then ships will attept to attack without waiting for the other ships that are on the way.
	...
	NAVAL_INVASION_PREPARE_HOURS = 100,								-- base hours needed to prepare an invasion
...
NAI = {
	...
	WANTED_UNITS_INDUSTRY_FACTORY = 1.25, 		-- How many units a country wants is partially based on how much military industry that is available
	...
	DIVISION_CREATE_MIN_XP = 505,				-- Minimum XP before attempting to create a fresh new division template.
	VARIANT_UPGRADE_MIN_XP = 505,				-- Minimum XP before attempting to create a new variant.
	...
	GIVE_STATE_CONTROL_MIN_CONTROLLED = 2,		-- AI needs to control more than this number of states before considering giving any away
	GIVE_STATE_CONTROL_MIN_CONTROL_DIFF = 3,	-- The difference in number of controlled states compared to war participation needs to be bigger than this for the AI to consider giving a state to a country
	...
	GIVE_STATE_CONTROL_DIFF_FACTOR = 1.0,		-- Diplo score multiplier for state control compared to war participation difference
	GIVE_STATE_CONTROL_NEIGHBOR_SCORE = 70,		-- Diplo score for each neighboring state controlled by the target
	GIVE_STATE_CONTROL_NEIGHBOR_ACTOR_SCORE = -90, -- Diplo score for each neighboring state that is controlled by the sender
	GIVE_STATE_CONTROL_NEIGHBOR_OTHER_SCORE = 30, -- Diplo score for each neighboring state controlled by someone else
	GIVE_STATE_CONTROL_MAX_SCORE_DIST = 100,	-- A State that is closer to the recipient capital than this gets a score bonus based on the below value
	GIVE_STATE_CONTROL_DIST_SCORE_MULT = 0.1,	-- Multiplier for the score gained from distance ( GIVE_STATE_CONTROL_MAX_SCORE_DIST - distance ) * this
	...
	GENERATE_WARGOAL_THREAT_BASELINE = 0.75,	    -- The baseline for what the AI considers the world is getting dangerous and we want to generate wargoals with no antagonize value	
	...
	INVASION_COASTAL_PROVS_PER_ORDER = 30,				-- AI will consider one extra invasion per number of provinces stated here (num orders = total coast / this)

	CONVOY_NEED_SAFETY_BUFFER = 2.5,					-- AI will try and keep 15% more convoys than what it needs.
	...
	MAX_DISTANCE_NALAV_INVASION = 400.0,				-- AI is extremely unwilling to plan naval invasions above this naval distance limit.
	...
	MIN_SUPPLY_USE_SANITY_CAP = 150,					-- Ignore supply cap if below this value when deciding on how many divisions to produce.
	...
	NEW_LEADER_EXTRA_PP_FACTOR = 1.0,					-- Country must have at least this many times extra PP to get new admirals or army leaders
	...
	AVERAGE_SUPPLY_USE_PESSIMISM = 2.5,					-- Multiplier for when AI calculates average supply use of entire army.
	...
	DEMOCRATIC_AI_FACTION_KICKING_PLAYER_THREAT_DIFFERENCE = 50.0, -- World threat generation difference needed to kick a player from a democratic faction
